...
function createPastMemberElem(member: PastMember, lang: string) {
  let resText = "";
  ...
  if (gradeMark === "d") {
    resText +=
      " (" + texts.members_past_grade_doctor[lang as "en" | "ja"] + ")";
  } else if (gradeMark === "m") {
    resText +=
      " (" + texts.members_past_grade_master[lang as "en" | "ja"] + ")";
  } else if (gradeMark === "b") {
    resText +=
      " (" + texts.members_past_grade_bachelor[lang as "en" | "ja"] + ")";
  } else if (gradeMark) {
    resText += " (" + gradeMark + ")";
  }
...
function createPastMemberList(pastMembers: PastMemberInfo[], lang: string) {
  return pastMembers.map((memberInfo, yearIdx) => {
    ...
    const yearMemberList = membersList.map((member, memberIdx) => (
      <div key={`${yearIdx}_${memberIdx}`}>
        {createPastMemberElem(member, lang)}
      </div>
    ));
  ...
  return (
    <div id="content_members">
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        <h2 className="text-2xl font-bold mb-6 mt-8">
          {texts.members_past_head[lang]}
        </h2>
        {createPastMemberList(pastMembers, lang)}
      </div>
    </div>
  );
}
